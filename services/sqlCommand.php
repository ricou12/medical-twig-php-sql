...
class MyComponentsSql {
    ...
    public function addNew ($lastname,$firstname, $birthdate,$phone,$mail){
        try {
    ...
    // public function listPatients($offset = "0"){
    //     try {
    //         $query = $this->_dataBase->prepare('select count(*) from patients');
    //         $query->execute();
    //         $result = $query->fetch();
    //         $query->closeCursor();
    //         $pages= ceil(intval($result[0]) / 5);
    //         $query = $this->_dataBase->prepare('select * from patients LIMIT 5 OFFSET :offset');
    ...
    public function rdvPatients($dateRdv,$heureRdv,$idpatients){
        try {
